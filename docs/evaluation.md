...
The checked-in fixture benchmark is also a regression gate. `scripts/eval_regression_gate.py` re-runs the fixture end to end and compares claim accuracy/macro F1, evidence F1/recall@K/MRR, abstention coverage, and average LLM calls per architecture against `evals/baselines/scifact-offline-fixture-smoke.json`. A drop fails CI; an improvement is reported; any change to dataset hashes, dataset revision, cache version, prompt versions, seed, split, or sample size is `BASELINE_DRIFT` and is never scored against stale numbers. The baseline is regenerated only with `--update-baseline` in a separate reviewed commit. The gate covers the deterministic fixture configuration only; it does not gate live-provider behavior.
